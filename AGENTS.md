...
4. 中文文案、导航文案、说明文案默认直接写中文，禁止无必要改成 `\uXXXX` 形式绕过乱码问题。
5. 若终端、脚本或工具显示乱码，不得直接据此覆盖源码；必须先用 UTF-8 复读原文件并确认真实内容后再修改。
6. 细则文档：[docs/rules/encoding.md](./docs/rules/encoding.md)
...
1. 每当新增功能时，必须先判断该功能属于客户端还是服务端。
2. 客户端功能通常落在页面交互、浏览器状态、前端组件与展示逻辑；服务端功能通常落在数据读取、鉴权、接口、服务端动作与 `src/lib/server` 侧能力。
3. 功能落地后，必须同步更新 `docs/project-directory-structure.md`，补充新增目录、入口文件、客户端/服务端归属和关键职责。
4. 目标是让 `docs/project-directory-structure.md` 始终与当前项目真实结构保持一致，不允许长期滞后1。
